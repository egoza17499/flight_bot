...
async def admin_stats_callback(callback: types.CallbackQuery):
    ...
    users = await get_all_users()
    total = len(users)
    banned_count = sum(1 for u in users if check_flight_ban(u))
...
async def admin_fill_airports_callback(callback: types.CallbackQuery):
    ...
    await callback.message.answer("⏳ Начинаю заполнение базы аэродромов... Это может занять 1-2 минуты.")
